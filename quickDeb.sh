...
echo https://mirrors.ustc.edu.cn/repogen/conf/$id-https-4-$codename | xargs $p curl -so /etc/apt/sources.list
...
# 安装软件
$p apt update
$p apt install git vim zsh python3 python3-pip python-is-python3 gcc make gdb openssh-server openssh-client -y
